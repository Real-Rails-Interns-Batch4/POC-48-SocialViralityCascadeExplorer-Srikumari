...
export default function SocialViralityCascadeExplorer() {
  ...
  // 🔄 CONFINES AUTO-SCROLL INSIDE TERMINAL CONSOLE ONLY
  useEffect(() => {
  ...
  // 🔌 LIVE ENVIRONMENT DATA INGESTION ENGINE
  useEffect(() => {
  ...
  // Assemble logging entries with GDELT identifiers
  useEffect(() => {
  ...
  // Playback timeline interval clock
  useEffect(() => {
  ...
  // 🎛️ CUSTOM NETWORK NODES BUILDER WITH GLASSMORPHISM AND HOVER TOOLTIPS
        ...
        data: { 
          label: (
            <div className="flex flex-col items-center justify-center p-2 text-center relative group">
              {/* 🛠️ GLASSMORPHISM HOVER NODE TOOLTIP */}
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover:flex flex-col w-56 p-2 bg-[#0B1117]/95 backdrop-blur-md border border-indigo-500/40 rounded shadow-2xl z-50 text-left text-[10px] font-mono text-slate-300 pointer-events-none">
                <div className="text-[#818CF8] font-bold border-b border-slate-800 pb-1 mb-1">{node.user}</div>
                <p className="italic text-slate-400">"{node.content}"</p>
                <div className="pt-1 mt-1 border-t border-slate-800 flex justify-between text-[9px] text-[#818CF8] font-bold">
  ...
  const flowEdges = useMemo<Edge[]>(() => {
    ...
    visibleRawNodes.forEach((node) => {
      if (node.parentId && visibleRawNodes.some(n => n.id === node.parentId)) {
        let edgeColor = "#818CF8"; // #818CF8 Indigo System Rail Accent
  ...
  return (
    // 🎨 MASTER SURFACE COLOR #0B1117 APPLIED UNIFORMLY
    <div suppressHydrationWarning className="h-screen w-screen bg-[#0B1117] text-slate-100 font-sans overflow-hidden flex flex-col">
      {/* Top Header Bar Row */}
      <header className="border-b border-slate-800/60 bg-[#090d16]/80 backdrop-blur-md px-6 py-3.5 flex items-center justify-between z-50 shrink-0">
        <div className="flex items-center space-x-3">
          <div className="h-3 w-3 bg-[#818CF8] rounded-full animate-pulse shadow-[0_0_10px_rgba(129,140,248,0.6)]" />
        ...
        <span className="text-xs font-mono uppercase bg-slate-800/60 text-slate-300 border border-slate-700/50 px-2.5 py-1 rounded flex items-center gap-1.5"><Radio size={11} className="text-[#818CF8] animate-pulse" /> GDELT Stream Rail</span>
      </header>
      ...
      <main className="flex flex-1 flex-row overflow-hidden w-full" suppressHydrationWarning>
        ...
        <section className="w-[70%] p-5 flex flex-col border-r border-slate-800/40 h-full overflow-y-auto space-y-4 scrollbar-thin">
          ...
          <div className="flex items-center justify-between border-b border-slate-800/40 pb-2 shrink-0" suppressHydrationWarning>
            ...
            <div className="flex items-center space-x-2">
              <select suppressHydrationWarning value={selectedScenario} onChange={(e) => { setSelectedScenario(e.target.value); setCurrentTime(0); }} className="bg-slate-900 border border-slate-800 text-xs text-white rounded px-3 py-1.5 focus:outline-none focus:border-[#818CF8]">
                <option value="A">Incident A: GDELT Global Security Leak</option>
                <option value="B">Incident B: GDELT Hardware Rumor Profile</option>
              </select>
              <select suppressHydrationWarning value={selectedPlatform} onChange={(e) => setSelectedPlatform(e.target.value)} className="bg-slate-900 border border-slate-800 text-xs text-slate-300 rounded px-3 py-1.5 focus:outline-none focus:border-[#818CF8]">
          ...
          <div className="relative h-[380px] bg-black/40 backdrop-blur-md rounded-lg border border-slate-800/80 overflow-hidden w-full shrink-0 shadow-inner">
          ...
          <div className="bg-[#090d16]/60 backdrop-blur-md border border-slate-800/80 rounded-lg p-3 space-y-2 shrink-0 w-full flex flex-col shadow-md">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                {/* 🛡️ HYDRATION SAFE NAVIGATION DECK BUTTON */}
                <button suppressHydrationWarning onClick={() => setIsPlaying(!isPlaying)} className="p-1.5 rounded flex items-center justify-center bg-[#818CF8] text-black font-bold hover:bg-[#818CF8]/80 transition-colors">{isPlaying ? <Pause size={14} /> : <Play size={14} className="ml-0.5" />}</button>
              ...
              <div className="flex items-center space-x-3 text-[10px] font-mono text-slate-400 bg-black/40 border border-slate-800/40 px-2.5 py-1 rounded">
                {/* 🎨 RESOLVED HEX SYNTAX COMPILATION BINDING INLINE FIX */}
                <div>Nodes: <span style={{ color: "#818CF8" }} className="font-bold">{currentMetrics.activeNodes}</span></div>
            ...
            <input type="range" min="0" max={maxTime} step="5" value={currentTime} onChange={(e) => setCurrentTime(parseInt(e.target.value))} className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-[#818CF8] focus:outline-none" />
          </div>
          ...
          <div className="bg-black/80 backdrop-blur-md border border-slate-800/90 rounded-lg p-3 font-mono text-xs flex flex-col shrink-0 h-[115px] w-full shadow-2xl overflow-hidden group">
            <div className="flex items-center justify-between border-b border-slate-900/80 pb-1.5 mb-1.5 text-slate-400 uppercase tracking-wider text-[9px] font-bold">
              <div className="flex items-center space-x-2">
                <Terminal size={11} className="text-[#818CF8]" />
                <span>GDELT Real-Time Intelligence Ticker Log</span>
              </div>
              <span className="text-[8px] bg-slate-900 border border-slate-800 px-1 rounded normal-case text-slate-500 group-hover:text-[#818CF8] transition-colors">↓ Scroll for Engineering Analytics</span>
            </div>
            <div className="flex-1 overflow-y-auto space-y-1 text-[11px] scrollbar-none text-slate-300">
              {logs.length === 0 ? <div className="text-slate-600 italic">Awaiting incoming GDELT stream metrics...</div> : logs.map((log) => (<div key={log.id} className="flex space-x-2"><span className="text-[#818CF8]/70 shrink-0">{log.timestamp}</span><span>{log.text}</span></div>))}
              <div ref={logEndRef} />
            </div>
          </div>
          ...
          <div className="w-full flex flex-col space-y-4 pt-4 border-t border-slate-900 shrink-0">
            {/* 📈 PLOT ROW 1: INDIGO ELEMENT PROPAGATION SPREAD TIMELINE */}
            <div className="space-y-1.5 w-full">
              <h4 className="text-[11px] font-semibold text-slate-300 uppercase font-mono flex items-center gap-1.5"><LineChart size={12} className="text-[#818CF8]" /> Node Propagation Spread Timeline</h4>
                ...
                <div className="flex-1 flex items-end justify-between space-x-4 pt-1 border-b border-slate-800/60 pb-1 relative">
                  {[
                    ...
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                      <span className="text-[9px] text-[#818CF8] font-mono font-bold scale-[0.85] mb-1">{col.n}</span>
                      <div className={`w-full ${col.h} bg-[#818CF8]/70 border border-[#818CF8]/20 rounded-t-sm transition-all group-hover:bg-[#818CF8]`} />
            ...
            {/* 📉 PLOT ROW 2: ENHANCED HALF-LIFE DECAY ANALYSIS */}
            <div className="space-y-1.5 w-full">
              <h4 className="text-[11px] font-semibold text-slate-300 uppercase font-mono flex items-center gap-1.5"><TrendingDown size={12} className="text-[#818CF8]" /> Ingestion Decay Half-Life Curves</h4>
              <div className="bg-slate-950/40 border border-slate-800/80 rounded-lg p-4 pl-10 h-56 flex flex-col justify-between relative overflow-hidden shadow-inner">
                ...
                <div className="flex-1 flex items-end justify-between pt-1 border-b border-slate-800/60 pb-1 relative z-10">
                  <svg className="absolute inset-0 w-full h-full p-2" viewBox="0 0 100 100" preserveAspectRatio="none">
                    ...
                    <path d="M 0 25 Q 40 85 100 98" fill="none" stroke="#818CF8" strokeWidth="1.5" />
                  </svg>
                  <div className="absolute left-3 top-3 text-[9px] text-pink-400/80 font-mono font-medium">Scenario A (High Ingestion Decay)</div>
                  <div className="absolute right-3 top-3 text-[9px] text-[#818CF8] font-mono font-medium">Scenario B (Stable Indigo Rail Line)</div>
                </div>
        ...
        {/* ========================================================================= */}
        <section className="w-[30%] bg-[#060a12]/70 backdrop-blur-md p-5 flex flex-col space-y-5 h-full overflow-y-auto border-l border-slate-800/40 justify-between shrink-0">
          <div className="space-y-5">
            <div>
              <h3 className="text-[10px] font-mono uppercase tracking-widest text-[#818CF8] font-bold">Dashboard Breakdown</h3>
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-900/50 border border-slate-800/80 rounded p-2.5 font-mono">
                  ...
                  <div className="text-xs font-bold text-[#818CF8] mt-1 truncate">{scoreboardMetadata.mainApp.split(" ")[0]}</div>
                </div>
              </div>
            ...
            <div className="space-y-1.5">
              <h4 className="text-xs font-semibold text-slate-300 uppercase font-mono flex items-center gap-1.5">
                <BarChart3 size={12} className="text-[#818CF8]" /> 1. Why This Matters
              </h4>
            ...
            <div className="space-y-1.5">
              <h4 className="text-xs font-semibold text-slate-300 uppercase font-mono flex items-center gap-1.5">
                <ShieldAlert size={12} className="text-[#818CF8]" /> 2. Who Controls the Rail
              </h4>
          ...
          <div className="pt-2 shrink-0">
            <button onClick={downloadSampleData} className="w-full py-2 bg-slate-900 border border-slate-800 hover:border-[#818CF8]/40 hover:bg-slate-800 rounded text-xs font-mono font-medium text-slate-300 flex items-center justify-center space-x-2 transition-all active:scale-[0.99]">
              <Download size={12} className="text-[#818CF8]" />
